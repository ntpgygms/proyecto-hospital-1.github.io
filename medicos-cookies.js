...
for (let i = 0; i < medicosCookie.length; i++) {
    ...
    let pacientesEncontrados = pacientesCookie.filter(paciente => medico.especialidad === paciente.especialidad);
    //se creo la celda
    ...
    if (pacientesEncontrados.length > 0) {
        ...
        for (let j = 0; j < pacientesEncontrados.length; j++) {
            ...
            pacientes.innerHTML += `<li>${pacienteEncontrado.nombre+" "+pacienteEncontrado.apellido}</li>`;
        }
    } else {
